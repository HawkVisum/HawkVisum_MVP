...
namespace UltimateXR.Manipulation.Helpers
{
    ...
    /// </summary>
    public class AllignOnReleaseMY : UxrComponent
    {
        ...
        private void Update()
        {
            if (!IsBeingGrabbed)
            {
                ...
                if (_onlyLevel == false)
                {
                    _selfTransform.localRotation = UxrInterpolator.SmoothDampRotation(_selfTransform.rotation, Quaternion.FromToRotation(_selfTransform.up, airplane.transform.up) * _selfTransform.rotation, _smoothFactor);
                }
                else
                {
